...
public class SparseIDESolver<N, D, M, V, I extends InterproceduralCFG<N, M>> {
    ...
    protected static final Logger logger = LoggerFactory.getLogger(heros.solver.IDESolver.class);
     ...
     *
     * @param callSite
     * @param d4       Fact stored with the incoming edge, i.e., present at the
     *                 caller side
        ...
        for (N m : icfg.getSuccsOf(n)) {
            ...
            for (D d3 : res) {
                ...
                if (nextUses.size() > 1) {
                    // propagate the d to the correct next stmt
                    for (N d3_nextUse : nextUses) {
                        if (d3_nextUse.equals(m)) {
                            //saveEdges(n, d3_nextUse, d2, res, false);
                            EdgeFunction<V> fprime = f.composeWith(edgeFunctions.getNormalEdgeFunction(n, d2, d3_nextUse, d3));
                            propagate(d1, d3_nextUse, d3, fprime, null, false);
                        }
                    }
                } else {
                    for (N d3_nextUse : nextUses) {
     ...
     *                           (this value is not used within this implementation but may be useful for subclasses of {@link heros.solver.IDESolver})
